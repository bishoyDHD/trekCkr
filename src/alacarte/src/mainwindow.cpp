...
#include <algorithm>
 ...
 MainWindow::MainWindow()
 {
   ...
   gl->addWidget(ffrom,2,0,1,3);
   gl->addWidget(fto,2,3,1,3);
...
void MainWindow::channelchange(int i)
{
  if (i<2) return;
  i-=2;
  if (i<db.model->infocolumns.size())
...
void MainWindow::writeTemplate()
{
  ...
  for (int i=0;i<db.model->infocolumns.size();i++)
    {
      templates->setArrayIndex(i);
  ...
  for (int i=0;i<db.model->detectorcolumns.size();i++)
    {
      templates->setArrayIndex(i);

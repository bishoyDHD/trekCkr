...
//      hist=new TH1D(mkdirs(buf),title, count, from,to); //Ievgen, March 29, 2018
...
void Plugin::saveValue(const char *tagname, const char *tablename, const char *valstring)
{
  ...
  int lengths[3]={(int)strlen(tagname),(int)sizeof(runnumber),(int)strlen(valstring)};
  int binary[3]={0,1,0};
  ...
  PQfinish(conn);
#else
  debug(10000,"DB not compiled in %s %s %s\n",tagname,tablename,valstring);
#endif
}

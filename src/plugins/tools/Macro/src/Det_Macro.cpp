...
Long_t Det_Macro::defineHistograms()
{
  ...
  if (inter->LoadFile(macroname.c_str()))
    {
  ...
  int erg;
  returnvalue=0;
  inter->Execute("defineHistograms",thisstring,&erg);
...
Long_t Det_Macro::startup()
{
 ...
 {  if (macroname=="")
  ...
  int erg;
  returnvalue=0;
  inter->Execute("startup",thisstring,&erg);
...
Long_t Det_Macro::process()
{
  ...
  int erg;
  inter->Execute("process",thisstring,&erg);
...
Long_t Det_Macro::finalize()
{
  ...
  int erg;
  returnvalue=0;
  inter->Execute("finalize",thisstring,&erg);
  return returnvalue;
}
